	...
	<table class="table" layoutH="162" width="100%">
		...
		<tbody>
			...
			<tr target="slt_uid" rel="${item.id}">
				...
				<td title="${item.ipAddress}">${item.ipAddress}</td>
				<td title="${item.logLevel}">${item.logLevel}</td>
				<td title="${item.message}">${item.message}</td>
				<c:if test="${not empty item.createTime}">
				<td title="<fmt:formatDate value="${item.createTime}" pattern="yyyy-MM-dd HH:mm:ss"/>"><fmt:formatDate value="${item.createTime}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
				</c:if>
