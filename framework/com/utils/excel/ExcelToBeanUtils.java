...
public class ExcelToBeanUtils
{
	 ...
	 *            数据入库对应的实体bean 对应
	 * @param fieldNames
	 *            字段名称列表，用于区分哪些字段需要获取图片对象
	 * @param isImage
	 *            true:导入的Excel中存在图片 ,false:无图片
	 * @return
	 * @throws Exception
	 */
	public static <T> List<T> readFilelModel(File file, Class<?> modelClass, Class<T> classzz, String[] fieldNames,
			boolean isImage) throws Exception
	{
		List<T> bList = new ArrayList<T>();
		
		if (null == file || null == modelClass)
		{
			return bList;
		}
		...
		try
		{
			...
			// 获取excel模型中与解析模型对应的属性名称
			refSupperFieldsName(salesVector, modelClass);
			
			// FileExcelUtils excelUtils = new FileExcelUtils<T>(set, fileType,
			// cls, file, imageFieldNams, isImage)
			
			// 构造解析参数
			FileExcelUtils<?> fileBean = new FileExcelUtils<T>(salesVector, FileExcelUtils.EXCEL_TYPE, modelClass,
					file, fieldNames, isImage);
			
			// 开始解析并返回解析结果
			List<?> modelList = fileBean.fileDataList();
			...
			if (CollectionUtils.isNotEmpty(modelList))
			{
				...
				for (Object model : modelList)
				{
					...
					if (null != bean)
					{
						reflectSetValue(salesVector, model, bean);
						bList.add(bean);
					}
				...
				for (Field field : fields)
				{
					if (null != fieldNameList && fieldNameList.contains(field.getName()))
					{
						...
						try
						{
							...
							{
								// 赋值
								field.set(destObj, com.utils.reflection.ReflectionUtils.getFieldValue(sourceObj, field
										.getName()));
