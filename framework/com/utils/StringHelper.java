...
public class StringHelper
{
	...
	public static String randomStr(int len) {
		String codeStr = "0123456789qwertyuioplkjhgfdsazxcvbnmQWERTYUIOPLKJHGFDSAZXCVBNM";
		len = len == 0 ? 5 : len;
		String rand = "";
		for (int i = 0; i < len; i++) {
			int codeLength = codeStr.length();
			int radom = (int) Math
					.round((Math.random() * 1000000) % codeLength);

			if (radom > codeLength - 1) {
				radom = (int) Math.round((Math.random() * 100) % len);
			}
			rand += codeStr.charAt(radom);
		}
		return rand;
	}

	public static String randomNum(int len) {
		String codeStr = "0123456789";
		len = len == 0 ? 5 : len;
		String rand = "";
		for (int i = 0; i < len; i++) {
			int codeLength = codeStr.length();
