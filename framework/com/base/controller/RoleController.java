...
import com.base.service.RoleBrandService;
...
public class RoleController {
	...
	private static final String ASSIGN_BRAND = "management/security/role/assign_brand";
	...
	@RequestMapping(value="/assignBrand/{id}", method=RequestMethod.GET)
	public String preAssignBrand(@PathVariable Long id, Map<String, Object> map) {
		Role role = roleService.get(id);
		
		List<RoleBrand> roleBrands = role.getRoldBrand();
		List<Long> rowBrand = new LinkedList<Long>(); 
		 
		for(RoleBrand roleBrand : roleBrands){
			rowBrand.add(roleBrand.getBrand().getId()); 
		}
		...
		map.put("rowBrand", rowBrand);
		return ASSIGN_BRAND;
	}
	
	
	@Log(message="修改了{0}角色的品牌。")
	@RequiresPermissions("Role:assign")
	@RequestMapping(value="/assignBrandSave/{id}", method=RequestMethod.POST)
	public @ResponseBody String assignBrandSave(@PathVariable Long id,Long[] ids) {
		Role role = roleService.get(id);
		List<RoleBrand> roleBrand = roleBrandService.findByRoleId(id);
		 
		for(RoleBrand rb : roleBrand){
			roleBrandService.delete(rb.getId());
		}
		 
		for(Long brandId : ids){
			Brand brand = brandService.get(brandId);
			RoleBrand perRoleBrand = new RoleBrand();
			perRoleBrand.setBrand(brand);
			perRoleBrand.setRole(role);
			roleBrandService.saveOrUpdate(perRoleBrand);
		}
		  
		return AjaxObject.newOk("分配品牌权限成功！").setCallbackType("").toString();
	}
